...
class AnnotationVisualizer:
    ...
    def visualize_single_document(self, image_file, annotation_file):
        ...
        # Save the output
        output_path = os.path.join(self.output_dir, f'{os.path.splitext(image_file)[0]}_annotated.png')
...
def main():
    ...
    image_files = [f for f in os.listdir(visualizer.images_dir) if f.endswith('.png')][:number_of_documents]
    for image_file in image_files:
        base_name = os.path.splitext(image_file)[0]
        annotation_file = f"{base_name}.json"
