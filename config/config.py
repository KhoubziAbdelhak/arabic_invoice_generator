...
LILT_ANNOTATIONS_DIR = os.path.join(OUTPUT_DIR, 'lilt_annotations')

# * Create directories if they don't exist
for directory in [TEMPLATE_DIR, OUTPUT_DIR, DOCX_DIR, PDF_DIR, IMAGES_DIR, ANNOTATIONS_DIR, LILT_ANNOTATIONS_DIR]:
...
VAT_RATE = 0.15
NUM_PRODUCTS_PER_INVOICE = 4
NUM_INVOICES_TO_GENERATE = 20
NUM_AUGMENTED_IMAGES = NUM_INVOICES_TO_GENERATE * 3
...
def validate_template(template_path):
    try:
        ...
        if template_name in auto_parts_templates or template_name == "generated_templates_1.docx":
            required_placeholders = {
                '{{seller_trade_name_ar}}', '{{seller_trade_name_en}}',
                '{{seller_location_line}}', '{{branch_name}}',
                '{{seller_vat_number}}', '{{issue_datetime}}', '{{issue_time}}',
                '{{invoice_type}}', '{{invoice_number}}',
                '{{customer_name}}', '{{customer_vat_number}}',
                '{{customer_address}}', '{{recipient_phone}}',
                '{{products_table}}', '{{subtotal}}', '{{discount}}',
                '{{net_amount}}', '{{tax}}', '{{total}}',
                '{{pos_phone_number}}', '{{pos_phone_number_2}}'
            }
        else:
            required_placeholders = {
...
for template_file in os.listdir(TEMPLATE_DIR):
    if not template_file.endswith(".docx") or template_file.startswith(".~lock"):
        continue
